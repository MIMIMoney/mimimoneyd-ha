[![NPM](https://nodei.co/npm/mimimoneyd-ha.png?downloads=true&stars=true)](https://nodei.co/npm/mimimoneyd-ha/)

[![Build Status](https://travis-ci.org/brandonlehmann/mimimoneyd-ha.png?branch=master)](https://travis-ci.org/brandonlehmann/mimimoneyd-ha) [![Build Status](https://ci.appveyor.com/api/projects/status/github/brandonlehmann/mimimoneyd-ha?branch=master&svg=true)](https://ci.appveyor.com/project/brandonlehmann/mimimoneyd-ha/branch/master)

# MIMIMoneyd High-Availability Daemon Wrapper

This project is designed to wrap the MIMIMoneyd daemon on a *nix system and monitor it for hangups, locks, fork, or other events that cause the daemon to stop responding to requests in an accurate manner.
...
5. [Documentation](#documentation)
   1. [Methods](#methods)
   2. [Events](#events)
   3. [MIMIMoneyd RPC API Interface](#mimimoneyd-rpc-api-interface)
...
* [MIMIMoneyd](https://github.com/turtlecoin/turtlecoin/releases) v0.19.0 or higher

## Easy Start

You *must* copy ```MIMIMoneyd``` into the ```mimimoneyd-ha``` folder for the easy start process to occur.

```bash
git clone https://github.com/turtlecoin/mimimoneyd-ha.git
cd mimimoneyd-ha
cp <MIMIMoneyd> .
sudo npm install & npm start
```

The installation will also download the latest checkpoints. Please see [MIMIMoney Checkpoints](http://checkpoints.turtlecoin.lol) for more information.
...
pm2 start service.js --name mimimoneyd
pm2 save
```
...
Practically all MIMIMoneyd command line arguments are exposed in the constructor method. Simply include them in your list of options to get activate or use them. Default values are defined below.

```javascript
var daemon = new MIMIMoneyd({
  // These are our MIMIMoneyd-ha options
  ...
  // These are the standard MIMIMoneyd options
  path: './MIMIMoneyd', // Where can I find MIMIMoneyd?
  dataDir: '~/.MIMIMoney', // Where do you store your blockchain?
...
This event is emitted when the daemon has lost synchronization with the MIMIMoney network
...
This event is emitted when the daemon is synchronized with the MIMIMoney network and is passing all the checks we have for it. It returns the equivalent of a */getinfo* call to the RPC server with a few minor additions.
...
This event is emitted when the daemon starts. The callback contains the command line arguments supplied to MIMIMoneyd.
...
This event is emitted when the daemon has synchronized with the MIMIMoney network.
...
Copyright (c) 2018, Brandon Lehmann, The MIMIMoney Developers
