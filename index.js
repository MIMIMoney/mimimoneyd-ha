...
const MIMIMoneydRPC = require('turtlecoin-rpc').MIMIMoneyd
...
const MIMIMoneyd = function (opts) {
  opts = opts || {}
  if (!(this instanceof MIMIMoneyd)) return new MIMIMoneyd(opts)
  ...
  */
  ...
  // Begin MIMIMoneyd options
  this.path = opts.path || path.resolve(__dirname, './MIMIMoneyd' + ((os.platform() === 'win32') ? '.exe' : ''))
  this.dataDir = opts.dataDir || path.resolve(os.homedir(), './.MIMIMoney')
  this.logFile = opts.logFile || path.resolve(__dirname, './MIMIMoneyd.log')
...
inherits(MIMIMoneyd, EventEmitter)

MIMIMoneyd.prototype.start = function () {
  ...
  this.emit('info', 'Attempting to start mimimoneyd-ha...')
...
MIMIMoneyd.prototype.stop = function () {
...
MIMIMoneyd.prototype.write = function (data) {
  this._write(util.format('%s\r', data))
}

MIMIMoneyd.prototype._checkChildStdio = function (data) {
...
MIMIMoneyd.prototype._triggerDown = function () {
...
MIMIMoneyd.prototype._triggerUp = function () {
...
MIMIMoneyd.prototype._checkServices = function () {
  if (!this.synced) {
    this.synced = true
...
MIMIMoneyd.prototype._checkRpc = function () {
  return Promise.all([
    this.api.info(),
    this.api.height()
  ])
...
MIMIMoneyd.prototype._checkDaemon = function () {
...
MIMIMoneyd.prototype._write = function (data) {
  this.child.write(data)
}

MIMIMoneyd.prototype._buildargs = function () {
  var args = ''
...
MIMIMoneyd.prototype._setupAPI = function () {
  this.api = new MIMIMoneydRPC({
    host: this.rpcBindIp,
...
MIMIMoneyd.prototype._setupWebSocket = function () {
...
MIMIMoneyd.prototype._registerWebSocketClientEvents = function (socket) {
  var that = this
